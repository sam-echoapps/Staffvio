    ...
    class InvoiceCreateViewModel {
        constructor(args) {
            ...
            function getApprovedTimesheet() { 
                if(sessionStorage.getItem("draft")=="draft"){
                     ...
                     /*                var resultArray = [];
                            ...
                            if(data.length !=0){ 
                                ...
                                var invoiceList = [
                                  ...
                                  // Loop through the data array and add rows dynamically
                                  invoiceList.forEach(function (item) {
                                    ...
                                    // Populate the cells with data from the array
                ...
                const formattedID = currentYear.toString()+currentMonth.toString()+currentDay.toString(); 
                ...
                if(result[6] !== 'null'){
                        if (result[6][0][0] === '7') {
                            updateDueDate(7);
                        } else if (result[6][0][0] === '10') {
                            updateDueDate(10);
                        }else if (result[6][0][0] === '14') {
                            updateDueDate(10);
                        }else if (result[6][0][0] === '30') {
                            updateDueDate(30);
                        }else if (result[6][0][0] === '45') {
                            updateDueDate(45);
                        }else if (result[6][0][0] === '60') {
                            updateDueDate(60);
                        }else if (result[6][0][0] === '90') {
                            updateDueDate(90);
                        }else if(result[6][0][0] == '1000'){
                            const today = new Date();
                            // const todayString = "Mon Feb 23 2023 13:44:57 GMT+0530 (India Standard Time)";
                            ...
                            if (
                                ...
                                lastDay = 31;
                            } else if (currentMonth === 1) { // February
                            ...
                            const endOfMonthDate = new Date(year, currentMonth, lastDay);

                            const dueYear = endOfMonthDate.getFullYear();
                            const dueMonth = (endOfMonthDate.getMonth() + 1).toString().padStart(2, '0');
                            const dueDay = endOfMonthDate.getDate().toString().padStart(2, '0');

                            self.dueDate(`${dueYear}-${dueMonth}-${dueDay}`);
                        } else if(result[6][0][0] == '1015'){
                            calculateDueDate(15);
                        }else if(result[6][0][0] == '1030'){
                            calculateDueDate(30);
                        }else if(result[6][0][0] == '1045'){
                            calculateDueDate(45);
                        }else if(result[6][0][0] == '1060'){
                            calculateDueDate(60);
                        }
                }
            }
            ...
            self.confirmSaveDraft = function (event,data) {
                    document.querySelector('#openConfirmSaveDraft').open();                
            }
            ...
            self.invoiceSaveAsDraft = function (event,data) {
        ...
        //    docprint.focus(); 
                ...
                var resultString = self.timesheetIdList().join(',');
                $.ajax({
                    url: BaseURL + "/jpClientInvoiceSaveAsDraft",
                    type: 'POST',
                    data: JSON.stringify({
                        invoiceId : sessionStorage.getItem("invoiceId"),
                        comments : self.comments()
            ...
            $.ajax({
                    ...
                    success: function (result) {
                        ...
                        console.log(result)
                        console.log(data)
                        self.invoiceDate(data[14])
                        self.dueDate(data[15])
                        self.invoiceAdditionalAmount([]);
        ...
        self.invoicePublish = function (event,data) {
            if (self.uploadError() == '' && self.typeError() == '') {
                document.querySelector('#openConfirmPublish').close();  
            document.querySelector('#openInvoicePublishProgress').open();  
            console.log(self.timesheetIdList())
            $.ajax({
        ...
        //    docprint.focus(); 
            ...
            self.invoiceUplaod = function (event,data) {
                self.uploadError('')        
                var file = event.detail.files[0];
                const result = event.detail.files;
                const files = result[0];
                var InvoiceFileName= files.name;
                ...
                reader.onload = ()=>{
                    ...
                    $.ajax({
                        url: BaseURL + "/jpInvoiceFileUplaod",
                        type: 'POST',
                        data: JSON.stringify({
                            invoiceId : sessionStorage.getItem("invoiceId"),
